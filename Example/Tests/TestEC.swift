...
class TestEC: XCTestCase {
    func testECPrivateKeyNew() {
        let f = base16 >>> toData >>> newECPrivateKey >>> rawValue >>> toBase16
    ...
    func testToECPublicKey() {
        let f = { base16 >>> toData >>> toECPrivateKey >>> toECPublicKey(isCompressed: $0) >>> rawValue >>> toBase16 }
    ...
    func testToECPaymentAddress() {
        let f = { base16 >>> toData >>> toECPublicKey >>> toECPaymentAddress(version: $0) }
    ...
    func testWIFToECPrivateKey() {
        let f = wif >>> toECPrivateKey >>> rawValue >>> toBase16
    ...
    func testWIFToECPublicKey() {
        let f = wif >>> toECPrivateKey >>> toECPublicKey >>> rawValue >>> toBase16
        let g = { wif >>> toECPrivateKey >>> toECPublicKey(isCompressed: $0) >>> rawValue >>> toBase16 }
    ...
    func testMessageSign() {
        XCTAssert("Nakomoto" |> toUTF8 |> signMessage(with: "KwE19y2Ud8EUEBjeUG4Uc4qWUJUUoZJxHR3xUfTpCSsJEDv2o8fu" |> wif) == "HxQp3cXgOIhBEGXks27sfeSQHVgNUeYgl5i5wG/dOUYaSIRnnzXR6NcyH+AfNAHtkWcyOD9rX4pojqmuQyH79K4=")
        XCTAssert("Nakomoto" |> toUTF8 |> signMessage(with: "5HpMRgt5u8yyU1AfPwcgLGphD5Qu4ka4v7McE4jKrGNpQPyRqXC" |> wif) == "GxQp3cXgOIhBEGXks27sfeSQHVgNUeYgl5i5wG/dOUYaSIRnnzXR6NcyH+AfNAHtkWcyOD9rX4pojqmuQyH79K4=")
    }
