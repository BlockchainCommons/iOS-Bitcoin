...
class TestBech32: XCTestCase {
    private let _validChecksum: [Bech32] = [
        "A12UEL5L",
    ...
    private let _invalidChecksum: [InvalidChecksum] = [
        (" 1nwldj5", Bech32Error.nonPrintableCharacter),
        ("\u{7f}1axkwrx", Bech32Error.nonPrintableCharacter),
        ("an84characterslonghumanreadablepartthatcontainsthenumber1andtheexcludedcharactersbio1569pvx", Bech32Error.stringLengthExceeded),
        ("pzry9x0s0muk", Bech32Error.noChecksumMarker),
        ("1pzry9x0s0muk", Bech32Error.incorrectHrpSize),
        ("x1b4n0q5v", Bech32Error.invalidCharacter),
        ("li1dgmt3", Bech32Error.incorrectChecksumSize),
        ("de1lg7wt\u{ff}", Bech32Error.nonPrintableCharacter),
        ("10a06t8", Bech32Error.incorrectHrpSize),
        ("1qzzfhee", Bech32Error.incorrectHrpSize)
    ]

    private let _invalidAddress: [Bech32] = [
    ...
    func testValidChecksum() {
        for valid in _validChecksum {
            do {
                let decoded = try valid |> toData
                XCTAssertFalse(decoded.hrp.isEmpty, "Empty result for \"\(valid)\"")
                let recoded = decoded.data |> toBech32(decoded.hrp)
    ...
    func testInvalidChecksum() {
        for invalid in _invalidChecksum {
            ...
            do {
                let decoded = try checksum |> toData
                XCTFail("Successfully decoded an invalid checksum \(checksum): \(decoded.data |> toBase16)")
    ...
    func testValidAddress() {
        for valid in _validAddressData {
            ...
            var decoded = try? address |> toSegwitAddress(hrp)

            do {
                if decoded == nil {
                    hrp = "tb"
                    decoded = try address |> toSegwitAddress(hrp)
                }
            } catch {
            ...
            XCTAssert(scriptPk == script, "Decoded script mismatch: \(scriptPk |> toBase16) != \(script |> toBase16)")

            do {
                let recoded = try decoded! |> toBech32(hrp)
    ...
    func testInvalidAddress() {
        for invalid in _invalidAddress {
            do {
                let decoded = try invalid |> toSegwitAddress("bc")
                XCTFail("Successfully decoded an invalid address \(invalid) for hrp \"bc\": \(decoded.program |> toBase16)")
            } catch {
                // OK here :)
            }

            do {
                let decoded = try invalid |> toSegwitAddress("tb")
                XCTFail("Successfully decoded an invalid address \(invalid) for hrp \"tb\": \(decoded.program |> toBase16)")
            } catch {
                // OK again
            }
        }
    ...
    func testInvalidAddressEncoding() {
        for invalid in _invalidAddressData {
            do {
                ...
                let wtf = try SegwitAddress(version: invalid.version, program: zeroData) |> toBech32(invalid.hrp)
    ...
    func testAddressEncodingDecodingPerfomance() {
        ...
        self.measure {
            do {
                for _ in 0..<10 {
                    let decoded = try addressToCode |> toSegwitAddress("bc")
                    let _ = try decoded |> toBech32("bc")
                }
            } catch {
    ...
    private func errorsEqual(_ lhs: Bech32Error, _ rhs: Bech32Error) -> Bool {
        switch lhs {
